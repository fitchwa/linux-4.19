...
{
	struct yaffs_timer		*bg_timer = from_timer(bg_timer, t, timer);
	wake_up_process(bg_timer->task);
}
...
static int yaffs_bg_thread_fn(void *data)
{
	...
	while (context->bg_running) {
		...
		//Y_INIT_TIMER(&timer);
		bg_timer.task = current;
		timer_setup(&bg_timer.timer, yaffs_background_waker, 0);
		
		bg_timer.timer.expires = expires + 1;
		//timer.data = (unsigned long)current;
		//timer.function = yaffs_background_waker;

		set_current_state(TASK_INTERRUPTIBLE);
		add_timer(&bg_timer.timer);
		schedule();
		del_timer_sync(&bg_timer.timer);
#else
		msleep(10);
#endif
	}
